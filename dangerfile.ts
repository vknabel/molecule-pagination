...
function isThisTestDisabled(namedDiff: NamedDiff): boolean {
  return (
    namedDiff.diff != null &&
    includes(namedDiff.diff.added, 'xit', 'xdescribe', 'fit', 'fdescribe')
  );
}
...
async function diffsForAllTests(): Promise<NamedDiff[]> {
  return Promise.all(modifiedTestFiles().map(namedDiffForFile));
}
...
function modifiedTestFiles(): string[] {
  return danger.git.modified_files.filter(fileName =>
    includes(fileName, 'testing/', '.mock.ts', '.spec.ts')
  );
}

function includes<T>(
  array: { indexOf: (item: T) => number },
  ...items: T[]
): boolean {
  return items.reduce(
    (included, item) => included || array.indexOf(item) !== -1,
    false
  );
}
